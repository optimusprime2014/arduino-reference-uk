---
title: Print
categories: [ "Functions" ]
subCategories: [ "Communication" ]
---




= Print


// OVERVIEW SECTION STARTS
[#overview]
--

[float]
=== Опис
Клас Print є абстрактним базовим класом, який надає загальний інтерфейс для друку даних на різні пристрої виводу. Він визначає кілька методів, які дозволяють друкувати дані в різних форматах.

Клас Print пов'язаний з кількома бібліотеками в Arduino, які використовують функціональність друку для взаємодії з такими пристроями, як монітор порту (Serial Monitor), РК-екран, принтери тощо.

Деякі з бібліотек, які використовують клас Print:

* link:https://www.arduino.cc/en/Reference/serial[Serial]
* link:https://reference.arduino.cc/reference/en/libraries/liquidcrystal/[LiquidCrystal]
* link:https://www.arduino.cc/en/Reference/Ethernet[Ethernet]
* link:https://reference.arduino.cc/reference/en/libraries/wifi/wificlient/[Wifi]


--
// OVERVIEW SECTION ENDS


// FUNCTIONS SECTION STARTS
[#functions]
--

'''

[float]
=== Функції
link:https://www.arduino.cc/reference/en/language/functions/communication/wire/write/[write()] +
link:https://www.arduino.cc/en/Reference/serial/print/[print()] +
link:https://www.arduino.cc/en/Reference/serial/println/[println()]

'''

--
// FUNCTIONS SECTION ENDS


// SEE ALSO SECTION
[#see_also]
--

[float]
=== Дивіться також

--
// SEE ALSO SECTION ENDS
